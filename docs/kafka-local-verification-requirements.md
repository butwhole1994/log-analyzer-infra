...
- `log-analyzer.dev.log-events` topic 생성 여부 확인 방법을 작성한다.
- `log-analyzer.dev.log-events-dlq` topic 생성 여부 확인 방법을 작성한다.
...
| `log-analyzer.dev.log-events-dlq` | `event-consumer` 처리 실패 메시지가 이동하는 DLQ topic |
...
- `log-analyzer.dev.log-events` topic의 Messages 화면에서 message payload를 확인한다.
- 새 메시지가 보이지 않으면 offset 조회 기준을 latest가 아닌 earliest 또는 beginning으로 변경한다.
- Consumer Groups 메뉴에서 `event-consumer` group을 확인한다.
- `log-analyzer.dev.log-events` topic의 end offset이 증가하는지 확인한다.
- consume 이후 consumer group의 current offset이 증가하고 lag이 감소하는지 확인한다.
- 실패 메시지가 발생한 경우 `log-analyzer.dev.log-events-dlq` topic에 메시지가 적재되는지 확인한다.
...
- `log-analyzer.dev.log-events` topic 생성 여부를 CLI로 확인할 수 있다.
- `log-analyzer.dev.log-events-dlq` topic 생성 여부를 CLI로 확인할 수 있다.
